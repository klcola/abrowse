ABrowse.view.StrandBedGraphView = function (genomeBrowser) {
    ABrowse.view.View.call(this, genomeBrowser);
    this.trackBodyHeight = 100; // 100
    this.fontHeight = 10; // 10
};

ABrowse.view.StrandBedGraphView.prototype = new ABrowse.view.View();

ABrowse.view.StrandBedGraphView.prototype.drawVerticalScaleMark = function (blockSvgGroup, maxValueInBlock) {
  var ORIGINAL_POINT_X = this.genomeBrowser.originalPointX;
  var markNum = 4;  // 5  （不显示最后一条线）
  var unitScore = (maxValueInBlock / (markNum / 2)).toFixed(2); // 刻度数字

  var scaleMarkSvgGroup = document.createElementNS(ABrowse.SVG_NS, "g");
  var scalePixels = (this.trackBodyHeight / markNum).toFixed(0);   // 每个刻度占多高
  ...
  for (var idx = 0; idx < markNum; ++idx) {
       ...
       var y = idx * scalePixels;  // idx * scalePixels
       var pointsArray = [];
       ...
       text.textContent = (unitScore * (markNum / 2 - idx)).toFixed(2) === 'NaN' ? '' : (unitScore * (markNum / 2 - idx)).toFixed(2);
...
ABrowse.view.StrandBedGraphView.prototype.drawBlock = function (blockResponse, trackName, trackSvgGroup) {
    ...
    entries.sort(function (a, b) { //  start 从小到大排列
    ...
    var x2, y2, x2Minus, y2Minus;
    for (var idx = 0; idx < entries.length; ++idx) {
        var entry = entries[idx];
        var start = entry.start;
        var length = entry.end - entry.start;
        var value = entry.value;
        var strand = entry.strand;


        var x1 = start - ORIGINAL_POINT_X;
        var x1Minus = start - ORIGINAL_POINT_X;

        var y1, y1Minus

        if (strand === '+') {
          pointsArray.push([x1, (this.trackBodyHeight ) / 2].join(" ")); // this.trackBodyHeight: 100
          y1 = (this.trackBodyHeight ) / 2 - ((value / maxVal ) * (this.trackBodyHeight ) / 2).toFixed(0);
        ...
        if (strand === '-') {
          pointsArray2.push([x1Minus, (this.trackBodyHeight ) / 2].join(" "));
          y1Minus = (this.trackBodyHeight ) / 2 + parseFloat(((value / maxVal ) * (this.trackBodyHeight ) / 2).toFixed(0));

          pointsArray2.push([x1Minus, y1Minus].join(" "));
          if (length > 1) {
              x2Minus = x1Minus + length - 1;
              y2Minus = y1Minus;
              pointsArray2.push([x2Minus, y2Minus].join(" "));
          }
        }
    }

    pointsArray.push([x2,(this.trackBodyHeight ) / 2].join(" "));
    pointsArray.push([x1, (this.trackBodyHeight ) / 2].join(" "));

    pointsArray2.push([x2Minus, (this.trackBodyHeight ) / 2].join(" "));
    pointsArray2.push([x1Minus, (this.trackBodyHeight ) / 2].join(" "));
    ...
    var polyline2 = document.createElementNS(ABrowse.SVG_NS, "polyline");
    polyline2.setAttribute("points", pointsArray2.join(" "));
    polyline2.setAttribute("fill", "blue");
    blockSvgGroup.appendChild(polyline2);
    pointsArray2 = [];
...
ABrowse.view.StrandBedGraphView.prototype.render = function (trackResponse, top) {
    ...
    for (var j = 0; j < blockResponses.length; ++j) {
        // let entriesPlus = JSON.parse(JSON.stringify(blockResponses[j].entryList))
        // entriesPlus.map(item => {
        //   item.strand = '+'
        // })
        // let entriesMinus = JSON.parse(JSON.stringify(blockResponses[j].entryList))
        // entriesMinus.map((item) => {
        //   item.strand = '-'
        // })
        // let arr = [...entriesPlus, ...entriesMinus]
        // let plusArr = arr.filter(item => item.strand === '+')
        // let minusArr = arr.filter(item => item.strand === '-')
    ...
    trackBodySvgGroup.__abrowse__height = this.trackBodyHeight; // 设置 trackBodyHeight
...
ABrowse.view.StrandBedGraphView.prototype.updateMaxValue = function (trackSvgGroup) {
    var ORIGINAL_POINT_X = this.genomeBrowser.originalPointX;
...
ABrowse.view.StrandBedGraphView.prototype.slide = function (trackSvgGroup, slideStepInPixels, top) {
